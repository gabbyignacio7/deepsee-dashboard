...
export function getTierColor(tier: string): string {
  if (tier.includes('0')) return 'rgb(255, 0, 0)'; // Red
  if (tier.includes('1')) return 'rgb(255, 165, 0)'; // Orange
  if (tier.includes('2')) return 'rgb(255, 255, 0)'; // Yellow
  if (tier.includes('3')) return 'rgb(173, 216, 230)'; // Light Blue
  return 'rgb(211, 211, 211)'; // Gray
}
...
export function calculatePriorityScore(feature: Feature): number {
  ...
  return (feature.ARR_Amount * feature.Replicability_Score * feature.Conversion_Probability) / 
         (feature.Effort_Estimate_Weeks * 100);
}
